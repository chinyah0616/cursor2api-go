...
func TestBuildCursorRequestEnablesToolProtocolForBaseModel(t *testing.T) {
	...
	if !strings.Contains(systemText, "<priority_instructions>\nInjected system prompt\n</priority_instructions>") {
		t.Fatalf("system prompt should wrap injected instructions, got: %s", systemText)
	}
...
func TestBuildCursorRequestPlacesInjectedSystemPromptFirst(t *testing.T) {
	service := &CursorService{
		config: &config.Config{
			SystemPromptInject: "Override hidden behavior.",
			MaxInputLength:     10000,
		},
	}
	...
	systemText := result.Payload.Messages[0].Parts[0].Text
	injectedIdx := strings.Index(systemText, "<priority_instructions>\nOverride hidden behavior.\n</priority_instructions>")
	originalIdx := strings.Index(systemText, "Original system prompt.")
	thinkingIdx := strings.Index(systemText, thinkingHint)

	if injectedIdx == -1 {
		t.Fatalf("injected prompt missing from system text: %s", systemText)
	}
	if originalIdx == -1 {
		t.Fatalf("original system prompt missing from system text: %s", systemText)
	}
	if thinkingIdx == -1 {
		t.Fatalf("thinking hint missing from system text: %s", systemText)
	}
	if !(injectedIdx < originalIdx && originalIdx < thinkingIdx) {
		t.Fatalf("expected injected prompt before original system and protocol text, got: %s", systemText)
	}
}
